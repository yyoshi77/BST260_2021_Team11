...
```{r, warning=FALSE}
library(gridExtra)
#Comparing the distribution of times in Rio 2016 and Tokyo 2020 by using histogram
dat1<-dat %>% mutate(time_sec=period_to_seconds(time))
p1<-dat1 %>% 
  filter(olympic %in% "Rio2016") %>%
  ggplot(aes(time_sec))+
  geom_histogram(binwidth =70, color="black",fill="orange")+
  xlab("Rio2016")+
  ylab("Time")
p1
p2<-dat1 %>% 
  filter(olympic %in% "Tokyo2020") %>%
  ggplot(aes(time_sec))+
  geom_histogram(binwidth =70, color="black",fill="blue")+
  xlab("Tokyo2020")+
  ylab("Time")
p2
...
#Comparing the times in Rio 2016 and Tokyo 2020 by using box-plot.
dat1%>%
  ggplot(aes(X=factor(olympic), y=time))+
  geom_boxplot(aes(olympic,time))+
  scale_y_time()+
  #scale_y_continuous(breaks = c((5*(1:12))^2),trans = "sqrt")+
  xlab("Olympic")+
  ylab("Records" )+
  ggtitle("Marathon Record")

#Comparing the distribution of age in Rio 2016 and Tokyo 2020 by using histogram
p3<-dat1 %>% 
  filter(olympic %in% "Rio2016") %>%
  ggplot(aes(age))+
...
#making table1
library(tableone)
t1 <- CreateTableOne(data = dat1, 
                     vars = c("time_sec", "sex", "age","sb","dnf","lockdown"),factorVars = "continent",
                     strata = "olympic")
t1
...
labels <-  list(time_sec ~ "records at each Olympic game", 
                          sb ~ "season best of the athlete, n (%)",
                          dnf ~ "did not finish, n (%)",
                          age ~ "Age, mean year(SD)",
                          lockdown ~ "lockdown, n (%)")

tbl1 <- tbl_summary(data=dat1 %>% 
...
table1 <- dat1 %>% 
 select(time_sec,sb,dnf,age,sex,lockdown) %>% 
 #mutate(Male = factor(Male, labels = c("Male", "Female"))) %>% 
 tbl_summary(by = olympic, missing = "no",
             type = list(c(time_sec, age) ~ "continuous",
                         c(sb, dnf, sex, lockdown) ~ "dichotomous"),
             statistic = list(time_sec ~ "{mean} ± {sd}",
                              age ~ "{mean} ± {sd}"),
             digits = list(all_dichotomous() ~ c(0,1)),
             label = list(time_sec ~ "records at each Olympic game", 
                          sb ~ "season best of the athlete, n (%)",
                          dnf ~ "did not finish, n (%)",
                          age ~ "Age, mean year(SD)",
                          lockdown ~ "lockdown, n (%)"))%>% 
...
table1
#Linear regression
model<-lm(time_sec ~  as.factor(olympic),data=dat1)
summary(model)

model1<-lm(time_sec ~  as.factor(olympic)+as.factor(sb)+age+as.factor(sex)+as.factor(continent),data=dat1)
summary(model1)

dat2<-dat1%>%
      mutate(age2=if_else(age>=25&age<35,1,0),
             age3=if_else(age>35,1,0))
             
sum(dat2$age2)
sum(dat2$age3)

model2<-lm(time_sec ~  as.factor(olympic)+age2+age3+as.factor(sb)+as.factor(sex)+as.factor(continent),data=dat2)
summary(model2)
dat2 %>% lm(time_sec ~  as.factor(olympic)+age2+age3+as.factor(sb)+as.factor(sex)+as.factor(continent),data = .) %>% 
  tbl_regression(intercept = TRUE)

model3<-lm(time_sec ~  as.factor(olympic)+age2+age3+as.factor(sb)+as.factor(sex)+as.factor(continent)+as.factor(olympic)*as.factor(lockdown),data=dat2)
summary(model3)
