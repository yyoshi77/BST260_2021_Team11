...
We are also interested in seeing how COVID cases per population and other covariates can help us predict marathon records in Tokyo 2020 Olympics. Rather than doing prediction and getting the exact performance, we can simplify the task to doing classification. The first thing we have to do is to convert the continuous marathon record data into a binary variable. We can first decide a cutpoint, and then classify the marathon records bigger than this point into a “slower” group, and the records smaller than this point into a “faster” group. 
...
We utilize the training data to determine the optimal cutpoint by the R package `cutpointr`. We first create a label that assigns the top half ranking athletes as 1, and the second half ranking athletes as 0, and use this label to calculate the sensitivity and specificity for different cutpoints. We then choose the cutpoint that gives us the highest sensitivity plus specificity.
...
The above two graphs are plotting the sum of sensitivity and specificity (using the function `sum_sens_spec`) against different cutpoints. From this analysis, the optimal cutpoint for men is 8239, and the optimal cutpoint for women is 9339. We will use these cutpoints throughout our machine learning and bootstrapping models.
...
We try to compare two sets of original vs. bootstrapped methods:

1. Decision tree and random forest

2. Naive Bayes and bootstrapped version of Naive Bayes

For random forest, we just apply the random forest function. For Naive Bayes, we do the bootstrap manually. We first sample with replacement from the training data 100 times, each time with sample size equal to my training set. Then, for each resample, we fit a model and get the predicted class for each test data. We then use majority vote among these 100 models to determine the class of each data. 

We will first fit the models and then present the ROC curves and some statistics in the end to do some comparisons.
...
Accuracy <- c("0.5", "0.7727", "0.6818", "0.6818")
Sensitivity <- c("0.5000", "0.8333", "0.8333", "0.8333")
Specificity <- c("0.5000", "0.7000", "0.5000", "0.5000")
PPV <- c("0.5455", "0.5714", "0.6667", "0.6667")
NPV <- c("0.4545", "0.5000", "0.7143", "0.7143")
AUC <- c("0.6083", "0.7", "0.7", "0.6583")
...
We can now compare the result of decision tree to random forest, and Naive Bayes to the bootstrapped version of Naive Bayes. If we focus on the accuracy and the AUC, we discovered bootstrapping is only helpful in one of the four models: the women case of tree and random forest. The accuracy increases from 0.5 to 0.7727, and AUC increases from 0.6083 to 0.7. For the 3 other cases, bootstrapping either doesn't have effect on the performance or is making the result even less accurate. 

This result is quite different from our expectation, since we are hoping that bootstrapping can help improve the performance our classification models a bit. Perhaps this is still due to our small sample size that causes bootstrapping to be not as useful. 
